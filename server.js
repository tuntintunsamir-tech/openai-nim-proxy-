...
const MODEL_MAPPING = {
  ...
  // 🤖 GLM/ChatGLM (Zhipu AI) - Chinese LLM ✅
  'glm-4': { model: 'zhipuai/chatglm3-6b', provider: 'nvidia' },
  'chatglm3': { model: 'zhipuai/chatglm3-6b', provider: 'nvidia' },
  'chatglm': { model: 'zhipuai/chatglm3-6b', provider: 'nvidia' },
...
app.post(['/chat/completions', '/v1/chat/completions'], async (req, res) => {
  try {
    ...
    if (provider === 'groq') {
      ...
      apiKey = GROQ_API_KEY;
      timeout = customTimeout || 60000; // 1 minute for Groq
      if (!apiKey) {
      ...
      apiKey = NIM_API_KEY;
      timeout = customTimeout || 180000; // 3 minutes default, or custom timeout
      if (!apiKey) {
        return res.status(500).json({
...
app.listen(PORT, () => {
  ...
  console.log(`🤖 GLM: glm-4, chatglm3, chatglm`);
